...
const ManageOrders = () => {
  ...
  const handleDeliver = (orderId) => {
    ...
    toast.success("تم نقل الأوردر لسجل المبيعات بنجاح! ✅");
  };

  // --- فانكشن رسالة التأكيد الشيك ---
    ...
    <div className="p-8 font-['Cairo']" dir="rtl">
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-3xl font-black dark:text-white flex items-center gap-3">
          <FaBox className="text-indigo-600" /> إدارة الطلبات النشطة
        </h2>
              ...
              <tr key={order.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-all">
                ...
                <td className="p-6">
                  <div className="flex flex-col gap-1">
                    <span className="font-black dark:text-white text-lg">{order.customerName}</span>
                    <span className="text-xs text-gray-400 font-bold tracking-widest">{order.phone}</span>
                  </div>
                </td>
                
                {/* عرض المنتجات بصورها وكمياتها */}
                <td className="p-6">
                    ...
                    {order.items.map((item, i) => (
                      <div key={i} className="flex items-center gap-3 bg-gray-50 dark:bg-gray-700/40 p-2 rounded-2xl border border-gray-100 dark:border-gray-600 w-fit min-w-[200px]">
                        ...
                        <div className="flex flex-col">
                          <span className="text-xs font-black dark:text-gray-200">{item.title?.slice(0, 20)}...</span>
                      ...
                      <>
                        ...
                        {order.walletNumber && (
                          <div className="flex items-center gap-1 text-green-600 font-black text-[11px] bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded-md border border-green-100 dark:border-green-800">
                            <FaPhoneAlt size={9} />
                            <span className="tracking-tighter">{order.walletNumber}</span>
                          </div>
                        )}
                      </>
                    ) : (
                ...
                <td className="p-6 text-center">
                  <div className="flex flex-col items-center">
                    <span className="font-black text-indigo-600 dark:text-indigo-400 text-xl">
                      LE {Number(order.totalAmount).toFixed(2)}
                    </span>
                  </div>
                </td>

                {/* الإجراءات (الصح والحذف بالشكل الجديد) */}
                <td className="p-6">
                  <div className="flex items-center justify-center gap-2">
                    <button 
                      onClick={() => handleDeliver(order.id)}
                      className="p-3 bg-green-100 text-green-600 rounded-2xl hover:bg-green-600 hover:text-white transition-all shadow-sm active:scale-90"
                    >
                      <FaCheck size={14} />
                    </button>
                    <button 
                      onClick={() => confirmReturn(order.id)} // استدعاء الرسالة الشيك
